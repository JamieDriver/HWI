...
def jade_exception(f):
    def func(*args, **kwargs):
        try:
            ...
            elif e.code in [-32600, -32601, -32001]:  # CBOR_RPC_INVALID_REQUEST, CBOR_RPC_UNKNOWN_METHOD, CBOR_RPC_PROTOCOL_ERROR
...
class JadeClient(HardwareWalletClient):
        ...
        for n_vin, (txin, psbtin) in py_enumerate(zip(c_txn.vin, tx.inputs)):
            ...
            for pubkey, origin in psbtin.hd_keypaths.items():
                if origin.fingerprint == master_fp and len(origin.path) > 0:
                    # Our input
                    if pubkey not in psbtin.partial_sigs or not psbtin.partial_sigs[pubkey]:
            ...
            if psbtin.non_witness_utxo:
                if txin.prevout.hash != psbtin.non_witness_utxo.sha256:
                    raise BadArgumentError('Input {} has a non_witness_utxo with the wrong hash'.format(n_vin))
    ...
    def display_singlesig_address(self, bip32_path: str, addr_type: AddressType) -> str:
        path = parse_path(bip32_path)
        addr_type = self._convertAddrType(addr_type)
        address = self.jade.get_receive_address(self._network(), path, variant=addr_type)
        return address
    ...
    def display_multisig_address(self, threshold: int, pubkeys: List[PubkeyProvider], addr_type: AddressType) -> str:
        ...
        """
        raise UnavailableActionError("The Blockstream Jade does not support generic multisig P2SH address display")
    ...
    def setup_device(self, label='', passphrase=''):
        ...
        :raises UnavailableActionError: Always, this function is unavailable
        """
        raise UnavailableActionError('The Blockstream Jade does not support software setup')
    ...
    def wipe_device(self):
        ...
        :raises UnavailableActionError: Always, this function is unavailable
        """
        raise UnavailableActionError('The Blockstream Jade does not support wiping via software')
    ...
    def restore_device(self, label='', word_count=24):
        ...
        :raises UnavailableActionError: Always, this function is unavailable
        """
        raise UnavailableActionError('The Blockstream Jade does not support restoring via software')
    ...
    def backup_device(self, label='', passphrase=''):
        ...
        :raises UnavailableActionError: Always, this function is unavailable
        """
        raise UnavailableActionError('The Blockstream Jade does not support creating a backup via software')
    ...
    def prompt_pin(self):
        ...
        :raises UnavailableActionError: Always, this function is unavailable
        """
        raise UnavailableActionError('The Blockstream Jade does not need a PIN sent from the host')
    ...
    def toggle_passphrase(self):
        ...
        :raises UnavailableActionError: Always, this function is unavailable
        """
        raise UnavailableActionError('The Blockstream Jade does not support toggling passphrase from the host')
